...
final class SalesViewModel: ObservableObject {
    // MARK: - Properties
    @Published private (set) var salesData = [SaleModel]()
    @Published private (set) var lastTotalSales: Int = 0
    ...
    func fetchNewSalesData() {
        salesData = SaleModel.threeMonthsExamples()
    }
